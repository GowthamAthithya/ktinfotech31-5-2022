...
const EditableTable = (props) => {
  ...
  const handleSubmit = async () => {
    ...
    const dataWithCurrentDate = getUserData?.date?.filter(
      (d) => d.date === Date
    );
    ...
    // const data =
    await updateDoc(usersIndividualRef, {
  ...
  const columns = [
    {
      ...
      render: (text) =>
        text ? (
          ...
          <input
            type="text"
            style={{ width: userData?.isAdmin ? "150px" : 0 }}
            value={text || name}
      ...
      render: (text) => {
        return (
          <Select
            style={{ width: "150px" }}
            placeholder="Status"
            defaultValue={text || wordStatus || "Not Available"}
      ...
      render: (text) => {
        return (
          <DatePicker
            ...
            defaultValue={moment(text ? text : completedDate, "DD/MM/YYYY")}
            format={"DD/MM/YYYY"}
          />
        );
      },
    ...
    <div>
      ...
      <div className="dashboard">
        ...
        <div className="dashboardContent">
          {userData?.isAdmin ? (
            <Button
              onClick={() => (addRow ? setAddRow(false) : setAddRow(true))}
              type="primary"
            >
              Add Task Row
            </Button>
